...
Lee Reporte_NetserGroup_Final.xlsx y genera index.html con dashboard interactivo.
...
CLIENT_ICONS = {
    "HP Comercial": "\U0001F5A8",
    "HPE": "\U0001F5A8",
    "Payless": "\U0001F6D2",
    "Netapp": "\u2601",
    "Lexmark": "\U0001F5A8",
    "Lexmark Kit": "\U0001F5A8",
    "CTDI": "\U0001F527",
    "Monthly Fee": "\U0001F4B2",
    "Lenovo": "\U0001F4BB",
}
...
def format_timestamp(val):
    ...
    s = str(val).strip()
    return s if s else datetime.now().strftime("%d/%m/%Y %H:%M:%S")
...
def read_excel():
    """Lee el archivo Excel y devuelve la estructura de datos."""
    print(f"[1/5] Leyendo archivo: {EXCEL_FILE}")
    ...
    if SHEET_NAME not in wb.sheetnames:
        print(f"[ERROR] No se encontro la hoja '{SHEET_NAME}'. Hojas disponibles: {wb.sheetnames}")
        sys.exit(1)

    ws = wb[SHEET_NAME]
    print(f"[2/5] Hoja '{SHEET_NAME}' encontrada")
    ...
    print(f"[3/5] {len(rows)} filas de datos leidas")
    return rows


def build_data(rows):
    """Construye la estructura JSON a partir de las filas del Excel."""
    print("[4/5] Construyendo estructura de datos...")
    ...
    for row in rows:
        ...
        for i, client in enumerate(CLIENTS):
            entry["clients"][client] = safe_int(row[1 + i])
        entry["total"] = safe_int(row[10])
    ...
    # Datos de hoy por cliente
    today = {}
    for i, client in enumerate(CLIENTS):
        today[client] = safe_int(latest[1 + i])

    total_cases = safe_int(latest[10])
    update_time = format_timestamp(latest[11])

    # Bots (columnas M-Z = indices 12-25)
    bots_data = []
    bots_ok = 0
    bots_fail = 0
    for i, bot_name in enumerate(BOTS):
        col_idx = 12 + i
        val = latest[col_idx] if col_idx < len(latest) else None
        status = is_bot_ok(val)
        bots_data.append({"name": bot_name, "status": "OK" if status else "FAIL"})
        if status:
            bots_ok += 1
        else:
            bots_fail += 1
    ...
    changes = {}
    for i, client in enumerate(CLIENTS):
        current = safe_int(latest[1 + i])
        prev = safe_int(previous[1 + i]) if previous else 0
        changes[client] = current - prev

    # Clientes activos (con casos > 0)
    active_clients = sum(1 for v in today.values() if v > 0)
...
def generate_html(data):
    ...
    print("[5/5] Generando index.html...")
...
<meta name="description" content="Dashboard Operacional NetserGroup - Monitoreo en Tiempo Real">
...
.kpi-change{{display:inline-flex;align-items:center;gap:3px;font-size:10px;font-weight:700;padding:2px 6px;border-radius:6px;margin-left:6px}}
.kpi-change.up{{color:#00B894;background:rgba(0,184,148,.1)}}
.kpi-change.down{{color:#E17055;background:rgba(225,112,85,.1)}}
.kpi-change.neutral{{color:var(--t4);background:rgba(61,90,128,.15)}}
...
.client-card.zero-card{{opacity:.55}}
.client-card.zero-card:hover{{opacity:.85}}
...
.client-change{{font-size:10px;font-weight:700;margin-top:4px;display:inline-flex;align-items:center;gap:2px;
  padding:1px 7px;border-radius:10px}}
.client-change.up{{color:#00B894;background:rgba(0,184,148,.12)}}
.client-change.down{{color:#E17055;background:rgba(225,112,85,.12)}}
.client-change.same{{color:var(--t4);background:rgba(61,90,128,.1)}}
...
.chart-wrap-lg{{position:relative;width:100%;height:380px}}
...
.bot-gauge-value{{font-size:36px;font-weight:900;color:#00B894}}
...
/* ====== HISTORY LEGEND ====== */
.history-legend{{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:12px;justify-content:center}}
.legend-item{{display:inline-flex;align-items:center;gap:5px;padding:4px 10px;
  background:var(--glass);border:1px solid var(--brd);border-radius:20px;cursor:pointer;
  font-size:10px;font-weight:600;color:var(--t2);transition:var(--transition);user-select:none}}
.legend-item:hover{{border-color:var(--a);background:rgba(0,180,216,.06)}}
...
/* ====== LIGHT THEME ====== */
body.light{{
  --bg:#F0F4F8;--bg2:#E2E8F0;--bg3:#CBD5E1;--bg-card:rgba(255,255,255,.75);
  --brd:#D1D9E6;--t1:#1E293B;--t2:#334155;--t3:#64748B;--t4:#94A3B8;
  --glass:rgba(0,0,0,.02);--glass2:rgba(0,0,0,.04);
  background:#F0F4F8;
}}
body.light #pc{{opacity:.08}}
body.light .ambient{{opacity:.3}}
</style>
</head>
<body>
...
<!-- ====== KPI STRIP ====== -->
<div class="kpi-strip" id="kpiStrip" data-aos="fade-up" data-aos-delay="100"></div>
...
<div class="section-header" data-aos="fade-right" data-aos-delay="50">
  ...
  <span class="section-badge" id="clientsBadge"></span>
</div>
...
<div class="grid-3" id="resumenGrid">
  <div class="panel" data-aos="fade-up" data-aos-delay="100">
    <div class="ring-container" style="position:relative">
      ...
      <div class="ring-center">
        <div class="ring-value counter" id="ringValue" data-target="0">0</div>
...
<div class="section-header" data-aos="fade-right">
  ...
  <span class="section-badge" id="historyBadge"></span>
</div>
<div class="panel" data-aos="fade-up" data-aos-delay="100" style="margin-bottom:8px">
  <div class="panel-title"><i data-lucide="trending-up"></i> Evolucion de Casos por Cliente</div>
  <div class="history-legend" id="historyLegend"></div>
  <div class="chart-wrap-lg"><canvas id="trendC"></canvas></div>
</div>
...
<div class="section-header" data-aos="fade-right">
  ...
  <span class="section-badge" id="botsBadge"></span>
</div>
<div class="grid-2">
  ...
  <div class="panel" data-aos="fade-up" data-aos-delay="200" style="display:flex;flex-direction:column;align-items:center;justify-content:center">
    ...
    <div class="bot-gauge">
      ...
      <div class="bot-gauge-center">
        <div class="bot-gauge-value" id="botGaugeValue">0%</div>
...
<div class="grid-4" id="opGrid"></div>
...
<footer>
  <p>Ultima Actualizacion: <span id="footerTime"></span> &mdash;
...
function renderDashboard() {{
  ...
  const bots = D.bots;
  const clients = D.clientOrder;
  const icons = D.clientIcons;
  const colors = D.chartColors;

  // --- KPI STRIP ---
  const kpiStrip = document.getElementById('kpiStrip');
  const totalChange = Object.values(changes).reduce((a,b) => a+b, 0);
  const changeArrow = (val) => val > 0 ? '<span class="kpi-change up">\\u2191 +'+val+'</span>' : val < 0 ? '<span class="kpi-change down">\\u2193 '+val+'</span>' : '<span class="kpi-change neutral">= 0</span>';

  kpiStrip.innerHTML = `
    <div class="kpi-mini success tooltip" data-tip="Bots funcionando correctamente">
      <div class="kpi-icon"><i data-lucide="bot"></i></div>
      <div class="kpi-data">
        <div class="kpi-value counter" data-target="${{meta.botsOk}}">0</div>
        <div class="kpi-label">Bots OK</div>
        <div class="kpi-sub">${{meta.botsOk}} / ${{meta.totalBots}} activos</div>
      </div>
    </div>
    <div class="kpi-mini danger tooltip" data-tip="Bots con errores">
      <div class="kpi-icon"><i data-lucide="alert-triangle"></i></div>
      <div class="kpi-data">
        <div class="kpi-value counter" data-target="${{meta.botsFail}}">0</div>
        <div class="kpi-label">Bots Fail</div>
        <div class="kpi-sub">${{meta.botsFail}} errores detectados</div>
      </div>
    </div>
    <div class="kpi-mini warning tooltip" data-tip="Alertas del sistema">
      ...
      <div class="kpi-data">
        ...
        <div class="kpi-sub">${{meta.botsFail === 0 ? 'Sin alertas pendientes' : meta.botsFail + ' alertas activas'}}</div>
      </div>
    </div>
    <div class="kpi-mini info tooltip" data-tip="Total de casos activos hoy">
      <div class="kpi-icon"><i data-lucide="briefcase"></i></div>
      <div class="kpi-data">
        <div class="kpi-value counter" data-target="${{meta.totalCases}}">0</div>
        <div class="kpi-label">Casos Activos ${{changeArrow(totalChange)}}</div>
        <div class="kpi-sub">${{meta.activeClients}} clientes activos</div>
      </div>
    </div>
  `;
  ...
  document.getElementById('clientsBadge').textContent = meta.totalClients + ' clientes';

  // Sort clients: active first (by cases desc), then inactive
  const sortedClients = [...clients].sort((a, b) => today[b] - today[a]);
  const maxCases = Math.max(...Object.values(today), 1);

  let cardsHTML = '';
  sortedClients.forEach((name, idx) => {{
    const val = today[name];
    const pct = meta.totalCases > 0 ? Math.round(val / meta.totalCases * 100) : 0;
    const barW = Math.round(val / maxCases * 100);
    const icon = icons[name] || '\\u2699';
    const isZero = val === 0;
    const isTop = idx === 0 && val > 0;
    const chg = changes[name] || 0;
    const chgClass = chg > 0 ? 'up' : chg < 0 ? 'down' : 'same';
    const chgText = chg > 0 ? '+' + chg : chg < 0 ? '' + chg : '=';

    cardsHTML += `
      <div class="client-card${{isTop ? ' active' : ''}}${{isZero ? ' zero-card' : ''}}" data-client="${{name}}" data-aos="zoom-in" data-aos-delay="${{50 + idx * 50}}">
        <div class="client-icon">${{icon}}</div>
        <div class="client-value${{isZero ? ' zero' : ''}} counter" data-target="${{val}}">0</div>
        <div class="client-name">${{name}}</div>
        ${{pct > 0 ? '<div class="client-pct">' + pct + '%</div>' : ''}}
        <div class="client-change ${{chgClass}}">${{chgText}}</div>
        <div class="client-bar"><div class="client-bar-fill" data-width="${{barW}}"></div></div>
      </div>
    `;
  }});
  ...
  // --- RING ---
  document.getElementById('ringValue').dataset.target = meta.totalCases;
  document.getElementById('ringSub').textContent = meta.activeClients + ' clientes activos de ' + meta.totalClients;
  ...
  bots.forEach(b => {{
    ...
    botHTML += `<tr><td>${{b.name}}</td><td><span class="${{isOk ? 'status-ok' : 'status-fail'}}"><i data-lucide="${{isOk ? 'check-circle' : 'x-circle'}}"></i> ${{b.status}}</span></td></tr>`;
  }});
  botTable.innerHTML = botHTML;
  document.getElementById('botsBadge').textContent = meta.totalBots + ' flujos monitoreados';

  // --- BOT GAUGE ---
  const successRate = meta.totalBots > 0 ? Math.round(meta.botsOk / meta.totalBots * 100) : 0;
  document.getElementById('botGaugeValue').textContent = successRate + '%';
  document.getElementById('botGaugeValue').style.color = successRate >= 80 ? 'var(--ok)' : successRate >= 50 ? 'var(--warn)' : 'var(--fail)';

  document.getElementById('botStats').innerHTML = `
    <div class="bot-stat"><div class="bot-stat-value" style="color:var(--ok)">${{meta.botsOk}}</div><div class="bot-stat-label">Exitosos</div></div>
    <div class="bot-stat"><div class="bot-stat-value" style="color:var(--fail)">${{meta.botsFail}}</div><div class="bot-stat-label">Fallidos</div></div>
    <div class="bot-stat"><div class="bot-stat-value" style="color:var(--a)">0</div><div class="bot-stat-label">Alertas</div></div>
  `;
  ...
  document.getElementById('opGrid').innerHTML = `
    <div class="op-kpi s" data-aos="flip-up" data-aos-delay="100">
      ...
      <div class="op-value counter" data-target="${{meta.botsOk}}">0</div>
      <div class="op-label">Exitosos</div>
      <div class="op-sub">${{meta.botsOk}} / ${{meta.totalBots}} bots</div>
    </div>
    <div class="op-kpi f" data-aos="flip-up" data-aos-delay="200">
      ...
      <div class="op-value counter" data-target="${{meta.botsFail}}">0</div>
      <div class="op-label">Fallidos</div>
      <div class="op-sub">${{meta.botsFail}} / ${{meta.totalBots}} bots</div>
    </div>
    <div class="op-kpi r" data-aos="flip-up" data-aos-delay="300">
      <div class="op-icon"><i data-lucide="star" style="color:var(--a)"></i></div>
      <div class="op-value counter" data-target="${{successRate}}">0</div>
    ...
    <div class="op-kpi m" data-aos="flip-up" data-aos-delay="400">
      ...
      <div class="op-value counter" data-target="${{meta.totalCases}}">0</div>
  ...
  // --- HISTORY BADGE ---
  document.getElementById('historyBadge').textContent = D.history.length + ' dias de datos';

  // --- FOOTER ---
  document.getElementById('footerTime').textContent = meta.updateTime;

  // Re-create Lucide icons for dynamically injected HTML
  lucide.createIcons();
}}
...
ScrollTrigger.create({{
  ...
  start: 'top 80%',
  onEnter: () => {{
    const total = DATA.meta.totalCases;
    const target = Math.max(total, 30);
...
// ====== CLIENT CARDS 3D TILT + MOUSE GLOW + CLICK ======
let barChart, doChart;
...
function setupClientCards() {{
  document.querySelectorAll('.client-card').forEach(card => {{
    ...
    card.addEventListener('click', () => {{
      ...
      highlightClientInCharts(clientName);
      document.getElementById('resumenGrid').scrollIntoView({{ behavior: 'smooth', block: 'center' }});
    }});
  }});
}}
setupClientCards();

function highlightClientInCharts(clientName) {{
  if (!barChart || !doChart) return;
  const clients = DATA.clientOrder;
  const activeClients = clients.filter(c => DATA.today[c] > 0);
  const idx = activeClients.indexOf(clientName);
  if (idx === -1) return;

  // Highlight in bar chart
  const bgColors = activeClients.map((c, i) => {{
    if (c === clientName) return DATA.chartColors[i % DATA.chartColors.length];
    return DATA.chartColors[i % DATA.chartColors.length] + '40';
  }});
  barChart.data.datasets[0].backgroundColor = bgColors;
  barChart.update();

  // Highlight in doughnut
  const doBg = activeClients.map((c, i) => {{
    if (c === clientName) return DATA.chartColors[i % DATA.chartColors.length];
    return DATA.chartColors[i % DATA.chartColors.length] + '30';
  }});
  doChart.data.datasets[0].backgroundColor = doBg;
  doChart.update();

  // Reset after 3 seconds
  setTimeout(() => {{
    barChart.data.datasets[0].backgroundColor = activeClients.map((c, i) => DATA.chartColors[i % DATA.chartColors.length]);
    barChart.update();
    doChart.data.datasets[0].backgroundColor = activeClients.map((c, i) => DATA.chartColors[i % DATA.chartColors.length]);
    doChart.update();
  }}, 3000);
}}
...
const activeClients = DATA.clientOrder.filter(c => DATA.today[c] > 0);
const activeValues = activeClients.map(c => DATA.today[c]);
const activeColors = activeClients.map((c, i) => DATA.chartColors[i % DATA.chartColors.length]);

// BAR CHART
const barCtx = document.getElementById('barC').getContext('2d');
const barColorPairs = [['#0D47A1','#1976D2'],['#1565C0','#2196F3'],['#1976D2','#42A5F5'],
  ['#1E88E5','#64B5F6'],['#2196F3','#90CAF9'],['#1A237E','#3F51B5'],
  ['#283593','#5C6BC0'],['#303F9F','#7986CB'],['#3949AB','#9FA8DA']];
const barBg = activeClients.map((c, i) => createGradient(barCtx, barColorPairs[i % barColorPairs.length]));
const barBorder = ['#1E88E5','#42A5F5','#64B5F6','#90CAF9','#BBDEFB','#5C6BC0','#7986CB','#9FA8DA','#C5CAE9'];
const totalCases = DATA.meta.totalCases;

barChart = new Chart(barCtx, {{
  type: 'bar',
  data: {{
    labels: activeClients,
    datasets: [{{
      data: activeValues,
      backgroundColor: barBg,
      borderColor: activeClients.map((c, i) => barBorder[i % barBorder.length]),
  ...
  options: {{
    ...
    plugins: {{
      ...
      tooltip: {{ backgroundColor: 'rgba(6,14,26,.95)', borderColor: '#1E3A5F',
        ...
        callbacks: {{ label: ctx => ctx.parsed.y + ' casos (' + (totalCases > 0 ? Math.round(ctx.parsed.y / totalCases * 100) : 0) + '%)' }} }}
    }},
    scales: {{
...
// DOUGHNUT CHART
doChart = new Chart(document.getElementById('doC'), {{
  type: 'doughnut',
  data: {{
    labels: activeClients,
    datasets: [{{
...
new Chart(document.getElementById('botD'), {{
  ...
  data: {{
    ...
    datasets: [{{ data: [DATA.meta.botsOk, DATA.meta.botsFail],
...
const trendColorsRGB = [
  '13,71,161','21,101,192','25,118,210','30,136,229','33,150,243',
  '26,35,126','40,53,147','48,63,159','57,73,171'
];
const hiddenClients = new Set();

// Build legend
const legendEl = document.getElementById('historyLegend');
DATA.clientOrder.forEach((name, i) => {{
  const dot = document.createElement('div');
  dot.className = 'legend-item';
  dot.dataset.client = name;
  dot.innerHTML = '<span class="legend-dot" style="background:' + trendColors[i % trendColors.length] + '"></span>' + name;
  dot.addEventListener('click', () => {{
    if (hiddenClients.has(name)) {{
      hiddenClients.delete(name);
      dot.classList.remove('hidden');
    }} else {{
      hiddenClients.add(name);
...
function buildTrendChart() {{
  const datasets = DATA.clientOrder.map((name, i) => {{
    const color = trendColors[i % trendColors.length];
    const rgb = trendColorsRGB[i % trendColorsRGB.length];
    return {{
      label: name,
      data: DATA.history.map(h => h.clients[name] || 0),
      borderColor: color,
      backgroundColor: 'rgba(' + rgb + ',.08)',
      borderWidth: 2,
      pointRadius: DATA.history.length > 30 ? 0 : 3,
      pointHoverRadius: 6,
  ...
  datasets.push({{
    ...
    borderColor: '#FDCB6E',
    backgroundColor: 'rgba(253,203,110,.06)',
    borderWidth: 3,
    borderDash: [6, 3],
    pointRadius: DATA.history.length > 30 ? 0 : 4,
    pointHoverRadius: 7,
    ...
    tension: 0.4,
    fill: false,
    hidden: hiddenClients.has('Total')
  }});
...
function createTrendChart() {{
  const ctx = document.getElementById('trendC').getContext('2d');
  trendChart = new Chart(ctx, {{
    type: 'line',
    data: {{
      labels: historyDates,
      datasets: buildTrendChart()
    }},
    options: {{
      responsive: true,
      ...
      plugins: {{
        ...
        tooltip: {{
          backgroundColor: 'rgba(6,14,26,.95)',
          borderColor: '#1E3A5F',
          borderWidth: 1,
          cornerRadius: 12,
          padding: 14,
          ...
          callbacks: {{
            label: function(ctx) {{
              if (ctx.parsed.y === 0 && ctx.dataset.label !== 'Total') return null;
              return ' ' + ctx.dataset.label + ': ' + ctx.parsed.y + ' casos';
            }}
          }}
        }}
      }},
      scales: {{
        ...
        y: {{
          grid: {{ color: 'rgba(30,58,95,.2)', drawBorder: false }},
          beginAtZero: true,
          ticks: {{ font: {{ size: 10 }} }}
        }}
      }}
...
(function() {{
  const dot = document.createElement('div');
  dot.className = 'legend-item';
  dot.dataset.client = 'Total';
  dot.innerHTML = '<span class="legend-dot" style="background:#FDCB6E"></span>Total';
  dot.addEventListener('click', () => {{
    if (hiddenClients.has('Total')) {{
      hiddenClients.delete('Total');
      dot.classList.remove('hidden');
    }} else {{
      hiddenClients.add('Total');
      dot.classList.add('hidden');
    }}
    updateTrendChart();
  }});
...
function updateTrendChart() {{
  ...
  trendChart.data.datasets = buildTrendChart();
  trendChart.update();
}}
...
document.getElementById('themeToggle').addEventListener('click', () => {{
  ...
  if (barChart) {{
    barChart.options.scales.x.ticks.color = darkMode ? '#5A7A9A' : '#64748B';
    barChart.options.scales.y.ticks.color = darkMode ? '#5A7A9A' : '#64748B';
    barChart.update();
  }}
  if (doChart) {{
    doChart.data.datasets[0].borderColor = darkMode ? '#0A1628' : '#F0F4F8';
    doChart.update();
  }}
}});

// ====== KPI / OP-KPI CARD HOVER ======
...
if __name__ == "__main__":
    print("=" * 50)
    print("  NetserGroup Dashboard Generator")
    print("=" * 50)
    print()
    ...
    print(f"  Datos: {len(data['history'])} dias historicos")
    print(f"  Clientes: {data['meta']['totalClients']} ({data['meta']['activeClients']} activos)")
    print(f"  Bots: {data['meta']['botsOk']}/{data['meta']['totalBots']} OK")
    print(f"  Total casos: {data['meta']['totalCases']}")
    print("=" * 50)
